...
<div class="ba_popup_compare">
	<p style="margin: 0px;">{l s='Added to your compare, View comparison list' mod='baproductscarousel'} <a style="color: #00aff0" href="{$baproductscarousel_base|escape:'htmlall':'UTF-8'}{$iso_lang|escape:'htmlall':'UTF-8'}/products-comparison" class="clish2" target="_blank">{l s='click here' mod='baproductscarousel'}</a></p>
		...
		{if count($shows) > 0}
			...
			<div style="width: {$sizeslide->sliw|escape:'htmlall':'UTF-8'};height: {$sizeslide->slih|escape:'htmlall':'UTF-8'}" class="fadeOut_{$id_sl|escape:'htmlall':'UTF-8'} owl-carousel owl-theme saddd {if $bablocks == 'home page tab'}owl-hidden{/if}">
				{foreach from=$shows item=item}
							...
							<div>
								...
								<div style="text-align: center;">
									{if $checkwishlist == true && $addtocart->wishlist == 1}
										<div class="whislist_casour wis {if BaProductsCarousel::selectWishList($item.id_product, $id_shop, $id_customer_ba) == 1}compare_check{/if}" id_s="{$item.id_product|escape:'htmlall':'UTF-8'}">
											<i class=""></i>
										</div>
									{/if}
									{if $addtocart->addcart == 1}
										<div class="add_to_carsou button-container" >
											<a {if Product::getQuantity($item.id_product) <= 0 } style="background: #C4C4C4 !important" {/if} class="ajax_add_to_cart_button" href="{$baproductscarousel_base|escape:'htmlall':'UTF-8'}{$iso_lang|escape:'htmlall':'UTF-8'}/cart?add=1&id_product={$item.id_product|escape:'htmlall':'UTF-8'}&token={$baproductscarousel_token|escape:'htmlall':'UTF-8'}" rel="nofollow" title="{l s='Add to cart' mod='baproductscarousel'}" data-id-product="{$item.id_product|escape:'htmlall':'UTF-8'}">
									...
									{if $addtocart->compare == 1}
										<div class="whislist_casour compare {if BaProductsCarousel::selectCompare($item.id_product, $id_customer_ba,$ba_compared_products) == 1}compare_check{/if}">
											<a {if BaProductsCarousel::selectCompare($item.id_product, $id_customer_ba,$ba_compared_products) == 1 } onclick="baslierremovecom(this)" {else} class="add_to_compare"{/if} data-id-product="{$item.id_product|escape:'htmlall':'UTF-8'}"><i class=""></i></a>
										</div>
									{/if}
								</div>
							</div>
					...
					{/if}
				{/foreach}
				{addJsDefL name=max_item}{l s='You cannot add more than %d product(s) to the product comparisons, view comparison list ' sprintf=$comparator_max_item  mod='baproductscarousel'}<a style="color: blue" href="{$baproductscarousel_base|escape:'htmlall':'UTF-8'}{$iso_lang|escape:'htmlall':'UTF-8'}/products-comparison" target="_blank">click here</a>{/addJsDefL}
				{addJsDefL name=added_to_wishlist}{l s='Added to your wishlist, view your wishlist ' mod='baproductscarousel'}<a class="clish2" style="color: #00aff0" href="{$baproductscarousel_base|escape:'htmlall':'UTF-8'}{$iso_lang|escape:'htmlall':'UTF-8'}/module/blockwishlist/mywishlist" target="_blank">{l s='click here' mod='baproductscarousel'}</a>{/addJsDefL}
