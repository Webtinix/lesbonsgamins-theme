...
<section>
  <p class="text-white">
    ...
    {if $bankwireCustomText }
        <a data-toggle="modal" id="bankwire-modal-btn" class="cursor-pointer" style="text-decoration: underline;">{l s='More information' d='Modules.Wirepayment.Shop'}</a>
    {/if}
  </p>
  ...
  <script>
    $('#bankwire-modal-btn').on('click', function(){
      $('#bankwire-modal').addClass('in');
      $('#bankwire-modal').show();
    })
    $('body').on('click',function(event){
        if($(event.target).is('#bankwire-modal')){
            $('#bankwire-modal').hide();
        }
    });
    $('#bankwire-modal .close').on('click', function(){
      $('#bankwire-modal').hide();
    })
  </script>
</section>