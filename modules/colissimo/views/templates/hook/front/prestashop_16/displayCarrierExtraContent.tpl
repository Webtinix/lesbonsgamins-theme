  ...
  {if isset($colissimo_customs_fees)}
    ...
    {else}
        <div class="customs-fees" style="display:none">
            <p class="alert alert-warning delivery-duty-paid"><strong>{l s='Delivery includes %s %s customs fees'  sprintf=[$colissimo_customs_fees, $currency] mod='colissimo'}</strong></p>
        </div>
    {/if}
  {/if}
