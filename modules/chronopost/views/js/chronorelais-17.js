...
function associateRelais(relaisID) {
    // Webtinix jimmy
    // J'ai commenté cette condition parce que la class .-current est retirée de la balise d'id #checkout-delivery-step
    // au clique sur un point relais, du coup la condition ne passe pas.
...
function initRelaisMarkers(address, cp, city, country) {
    // Webtinix
    // Jimmy a rajouter le panier ('&cartID=' + cartID) dans la requête pour pouvoir obtenir le dernier point relais selectionné
    let currentCarrier = $('.delivery-options input:checked').val();
    if (city === null) {
        $.ajax({
            url: path + '/async/getPointRelais2.php?city=unknown&codePostal=' + cp + '&country=' + country + '&carrier=' + currentCarrier + '&cartID=' + cartID,
            success: createAllPointRelais
        });
    } else {
        $.ajax({
            url: path + '/async/getPointRelais2.php?codePostal=' + cp + '&address=' + encodeURIComponent(address)
                + '&country=' + country + '&city=' + city + '&carrier=' + currentCarrier + '&cartID=' + cartID,
            success: createAllPointRelais
        });
    }
}
...
function buildMarkerPopup(relais) {
    var popup = '<div class="pointRelais"><p>'
        + relais.nom + '</p><p>' + relais.adresse1 + '<br/ >' + relais.codePostal
