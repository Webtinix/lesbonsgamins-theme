...
{block name='customer_form'}
  ...
  {* <div class="{if $page.page_name == "identity"}border rounded p-6{/if}"> *}
  <div class="{if $page.page_name == "identity"}flex{/if} {if $page.page_name == "identity"}w-full md:w-2/3 py-6{/if}">
    ...
    {* {$urls.base_url|dump} *}
    {* Insertion des informations du Gest *}
    {if (isset($gest_adress) && $gest_adress != "") && (isset($gest_email) && $gest_email != "")}
      {$formFields['email']['value'] = $gest_email}
      {$formFields['lastname']['value'] = $gest_adress.lastname}
      {$formFields['firstname']['value'] = $gest_adress.firstname}
      {$action = $urls.base_url|cat:"login?back=history"}
      {$formFields['email']['type']  = "hidden"}
      {$formFields['email']['show_value_for_hidden_field']  = $gest_email}
      {$formFields['email']['update_visible_value'] = '
    ...
    {* Insertion du mail pour le custom checkout *}
    {if isset($checkout_custom_step_email)}
      {if ($checkout_custom_step_is_customer_exist != true)}
        ...
        {$formFields['email']['type']  = "hidden"}
        {$formFields['email']['show_value_for_hidden_field']  = $checkout_custom_step_email}
        {$formFields['email']['update_visible_value'] = '
          <span class="edit-email-container">
            <a 
              @click="editEmail = true;"
        ...
        '}
        {*$formFields['email']['attr'] = ["disabled"]*}
      {/if}
    {/if}
    {* Insertion du mail pour le custom authetication *}
    {if isset($custom_authentication_email)}
      {if ($custom_authentication_is_customer_exist != true)}
        {$formFields['email']['value'] = $custom_authentication_email}
        {$formFields['email']['type']  = "show_value_for_hidden_field"}
        {$formFields['email']['additional_class'] = 'border-b-2 border-primary-focused grid grid-col-1'}
        {$formFields['email']['show_value_for_hidden_field']  = $custom_authentication_email}
        {$formFields['email']['update_visible_value'] = '
          <span class="edit-email-container">
            <a 
              @click="editEmail = true;"
    ...
    {$formFields['email']['additional_class_for_parent'] = "md:col-span-2 md:py-3"}
    {$formFields['password']['additional_class_for_parent'] = "md:col-span-2 md:py-3"}
    {$formFields['password']['availableValues']['placeholder'] = "Définir un nouveau mot de passe"}
    {$formFields['ps_emailsubscription_newsletter']['additional_class_for_parent'] = "md:col-span-2 md:py-3"}
    {* {$formFields['ps_emailsubscription_newsletter']['label'] = "Je souhaite être tenu informé des avantages promotionnels, des nouveaux produits, des conseils et des actualités Les Bons Gamins"} *}

    {* On désactive l'accès à la newsletter *}
    {* {$formFields['ps_emailsubscription_newsletter']['additional_class_for_parent']  = 'hidden'} *}
    <form action="{block name='customer_form_actionurl'}{$action}{/block}" id="customer-form" class="js-customer-form"
      ...
      <div class="grid grid-cols-1 grid-cols-1 md:grid-cols-2 gap-y-6 md:gap-4 ">
        {block "form_fields"}
          {* On mets une variable pour modifier la disposition des champs *}
          {assign var="formFieldsOrderList" value=[]}
          {* {$formFields|dump} *}
          {$formFieldsOrderList['email']=$formFields['email']}
          {$formFieldsOrderList['lastname']=$formFields['lastname']}
          {$formFieldsOrderList['firstname']=$formFields['firstname']}
          {$formFieldsOrderList['password']=$formFields['password']}
          {$formFieldsOrderList['ps_emailsubscription_newsletter'] = $formFields['ps_emailsubscription_newsletter']}
          {* {$formFields|dump} *}

          {foreach from=$formFieldsOrderList item="field"}
            {* {$field|dump} *}
          ...
          <ul class="list-disc mb-0">
            {if $errors|count}
              {foreach $errors as $error}
                {if isset($error[0])}
                  <li class="lbg-text-error mb-2">{$error[0]}</li>
                {/if}
              {/foreach}
            {/if}
            <div id="errors-block" class="">
              <li id="error-msg" class="lbg-text-error mb-2 hidden"></li>
            </div>
          </ul>
        {/block}
      </div>
      ...
      {block name='customer_form_footer'}
        <footer class="form-footer clearfix">
          ...
          {block "form_buttons"}
            <button class="w-full md:w-auto continue bg-lbg-primary hover:bg-primary text-white text-sm px-14 py-3 md:py-2 rounded-sm mt-3" data-link-action="save-customer" type="submit">
              {if isset($custom_authentication_email)}
